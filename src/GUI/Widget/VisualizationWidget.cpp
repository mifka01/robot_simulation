...
void VisualizationWidget::drawRobot(QPainter& painter,
  ...
  double eyeAngle =
      robot->getViewAngleRad() - M_PI / 6;  // Adjust eye angle as needed
  double eyeX1 =
      robot->getX() + robot->getDiameter() / 2 + eyeDistance * cos(eyeAngle);
  double eyeY1 =
      robot->getY() + robot->getDiameter() / 2 + eyeDistance * sin(eyeAngle);
...
void VisualizationWidget::mouseReleaseEvent(__attribute((unused))
                                            QMouseEvent* event) {
  if (grabbedObstacle) {
